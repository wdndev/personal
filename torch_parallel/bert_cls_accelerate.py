...
class Trainer:
    ...
    def __init__(self, args, model_engine, criterion, optimizer, accelerator):
        self.args = args  # 训练参数
    ...
    def test(self, model_engine, test_loader, labels):
        """ 测试
        """
        self.model_engine = model_engine  # 更新模型（可加载已保存的最佳模型）
        self.model_engine.eval()  # 设置模型为评估模式
...
class TrainArgs:
    ...
    ckpt_path = "output/bert_cls_accelerate_4.pt"
...
def main():
    ...
    model_engine, optimizer_engine, train_loader_engine, valid_loader_engine = accelerator.prepare(
        model, optimizer, train_loader, valid_loader
    )

    # =======================================
    # 定义训练器
    trainer = Trainer(args, model_engine, criterion, optimizer_engine, accelerator)
    
    # 训练和验证
    trainer.train(train_loader_engine, valid_loader_engine)
...
# nohup python -m torch.distributed.launch --nproc_per_node 2 --use_env bert_cls_accelerate.py > logs/bert_cls_accelerate_2.log 2>&1 &

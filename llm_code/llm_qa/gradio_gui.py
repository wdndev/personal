...
from qa_chain.qa_chain import QAChain, QAChainWithHistory
...
LLM_MODEL_DICT = {
    ...
    "zhipuai": ["chatglm_pro", "chatglm_std", "chatglm_lite", "chatglm3"]
}
...
AIGC_AVATAR_PATH = "images/aigc_avatar.png"
HUMAN_AVATAR_PATH = "images/human_avatar.jpg"

def get_model_platform(platform):
...
class QAModel():
    ...
    - qa_chain_history_dict: 以 (model, embedding) 为键存储的带历史记录的问答链。
    - qa_chain_dict: 以 (model, embedding) 为键存储的不带历史记录的问答链。
    """
    def __init__(self):
        self.qa_chain_history_dict = {}
        self.qa_chain_dict = {}

    def qa_chain_history_answer(self, 
                                question: str, 
                                chat_history: list = [], 
                                model: str = "openai", 
        ...
        if question == None or len(question) < 1:
            return "", chat_history
        try:
            if (model, embedding) not in self.qa_chain_history_dict:
                self.qa_chain_history_dict[(model, embedding)] = QAChainWithHistory(
                    model=model, 
            ...
            chain = self.qa_chain_history_dict[(model, embedding)]
            answer, history = chain.answer(question=question, temperature=temperature, top_k=top_k)
            return "", history
        except Exception as e:
            return e, chat_history

    def qa_chain_answer(self, 
                        question: str, 
                        chat_history: list = [], 
                        model: str = "openai", 
                        embedding="openai", 
        ...
        if question == None or len(question) < 1:
            return "", chat_history
        try:
            if (model, embedding) not in self.qa_chain_dict:
                self.qa_chain_dict[(model, embedding)] = QAChain(
                    model=model, 
                    temperature=temperature,
                    top_k=top_k, 
                    file_path=file_path, 
            ...
            chain = self.qa_chain_dict[(model, embedding)]
            chat_history.append(
    ...
    def clear_history(self):
        if len(self.qa_chain_history_dict) > 0:
            for chain in self.qa_chain_history_dict.values():
                chain.clear_history()
...
def format_chat_prompt(message, chat_history):
    ...
    for chat in chat_history:
        # 从聊天记录提取用户和机器人的消息
        user_message, bot_message = chat
        # 更新 prompt，加入用户和机器人的消息
        prompt = f"{prompt}\nUser: {user_message}\nAssistant: {bot_message}"
    # 将当前用户消息队列也加入prompt中，并预留一个位置给机器人的回复
    prompt = f"{prompt}\nUser: {message}\nAssistant:"
    # 返回格式化后的 prompt。
    return prompt
...
with block as demo:
    with gr.Row(equal_height=True):           
        with gr.Column(scale=1):
            gr.Markdown("""<h1><center>LLMs + ZeRO + RAG Demo</center></h1>""")

    with gr.Row():
        with gr.Column(scale=4):
            chatbot = gr.Chatbot(height=600, show_copy_button=True, show_share_button=True, avatar_images=(HUMAN_AVATAR_PATH, AIGC_AVATAR_PATH))
            # 创建一个文本框组件，用于输入 prompt。
            msg = gr.Textbox(label="Prompt/问题")

            with gr.Row():
                # 创建提交按钮。
                db_with_his_btn = gr.Button("检索对话历史问答")
                db_wo_his_btn = gr.Button("检索对话问答")
                llm_btn = gr.Button("大模型对话")
            with gr.Row():
                # 创建一个清除按钮，用于清除聊天机器人组件的内容。
                clear = gr.ClearButton(
                    components=[chatbot], value="清除对话框")
        ...
        db_with_his_btn.click(qa_model.qa_chain_history_answer, inputs=[
                              msg, chatbot, llm, embeddings, temperature, top_k, history_len], outputs=[msg, chatbot])
        # 设置按钮的点击事件。当点击时，调用上面定义的 qa_chain_self_answer 函数，并传入用户的消息和聊天历史记录，然后更新文本框和聊天机器人组件。
        db_wo_his_btn.click(qa_model.qa_chain_answer, inputs=[
        ...
        # 点击后清空后端存储的聊天记录
        clear.click(qa_model.clear_history)
    gr.Markdown("""提醒：<br>
